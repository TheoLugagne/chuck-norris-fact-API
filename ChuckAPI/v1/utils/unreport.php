...
function unreport(int $id): string
{
    if ($id === 0) {
    ...
    $responsePatch = json_decode(FactsDAO::patchChuckFact($linkPDO, $id, signalement: false), true);
    if ($responsePatch["status_code"] !== 200) {
        return deliver_response($responsePatch["status_code"], $responsePatch["status_message"]);
    }
    return deliver_response(200, "Retrait du signalement enregistré", $responsePatch["data"]);
}
